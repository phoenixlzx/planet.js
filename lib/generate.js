...
function restoreimg (attribsrc) {
    if (attribsrc.startsWith('/') && attribsrc.charAt(1) !== '/') {
        return host + attribs.src;
    ...
    return sanitizeHtml(content, {
        ...
        transformTags: {
            ...
            'img': function (tagName, attribs) {
                ...
                if (attribs) {
                    if (attribs.src) {
                        newimgtag.src = restoreimg(attribs.src);
                    }
                    if (attribs['data-src']) {
                        newimgtag['data-src'] = restoreimg(attribs['data-src']);
                    }
                }
                return newimgtag;
            },
